...
const createLinearProvider = (url, stateClass, context) => ({children}) => {
    ...
    useEffect(() => {
        axios.post(url + '/linear', stateClass)
        .then(response => setStates(response.data[0]))
        .then(() => setSource(new SSE(url + '/linear/updates', {payload: stateClass})))
    }, [])
...
export default createLinearProvider